...
class BareboneBuilder:
    def __init__(self, root):
        ...
        self.copy_button = tk.Button(self.root, text="process kill", command=self.copy_file)
        self.copy_button.pack(pady=5)
        self.text_area.delete(1.0, tk.END)
        self.text_area2.delete(1.0, tk.END)
        self.text_area.insert(tk.END,"python",True)

        try:
            
            result = subprocess.check_output("ps -a", stderr=subprocess.STDOUT, shell=True, text=True)
    ...
    def copy_file(self):
        ...
        for tx1 in txts2:
            ...
            for tx2 in txts3:
                tx2=tx2.strip()
                if tx2!="":
                    self.execute_command("killall "+tx2,True)
